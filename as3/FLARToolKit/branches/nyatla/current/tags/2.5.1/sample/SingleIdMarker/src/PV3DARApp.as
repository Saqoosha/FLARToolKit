...
import org.libspark.flartoolkit.core.transmat.*;
class MarkerProcesser extends FLSingleNyIdMarkerProcesser
{
	protected var _detector:MarkerProcesser;
	...
	{
		super();
		super.initInstance(i_param, new NyIdMarkerDataEncoder_RawBit(), 80);
	...
	{
		this.active = true;
		var data:NyIdMarkerData_RawBit =i_code as NyIdMarkerData_RawBit;
		var i:int;
		var current_id:int;
		if(data.length>4){
			//4バイト以上の時はint変換しない。
			...
			for(i=0;i<data.length;i++){
				current_id=(current_id<<8)|data.packet[i];
			}
		}
		trace("ID="+current_id);
		this.active = true;		
