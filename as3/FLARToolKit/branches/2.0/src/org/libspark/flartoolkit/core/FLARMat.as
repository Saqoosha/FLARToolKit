...
package org.libspark.flartoolkit.core {
	...
	public class FLARMat{
	     ...
	     * 
	     */
	    protected var m:Array;//double[][] m;
	    private var clm:int
		private var row:int;
	    
	    public function FLARMat(i_row:int, i_clm:int) {
			m = ArrayUtil.createMultidimensionalArray(i_row, i_clm);//new double[i_row][i_clm];
			clm = i_clm;
			row = i_row;
	    }
	...
	//    }
	    ...
	    private var wk_nos_matrixSelfInv:Array = new Array(50);//int[50];
//	    private static const matrixSelfInv_epsl:Number = 1.0e-10;
	    /**
...
//			int[] nos=wk_nos_matrixSelfInv;//この関数で初期化される。
	        ...
	        for (n = 0; n < dimen; n++) {
	            ...
	            p=0.0;
	            for (var i:int = n; i<dimen ; i++) {//for (i = n, wap = wcp, p = 0.0; i < dimen ; i++, wap += rowa)
	        ...
	        for (var i:int = 0; i < clm; i++) {
	            for (j = 0; j < clm; j++) {
	                if (j < i) {
	                    ...
	                    w=0.0;//*out = 0.0;
	                    for (k = 0; k < row; k++) {
	                        ina = input.m[k];//in=input.getRowArray(k);
	                        w += (ina[i] * ina[j]);//*out += *in1 * *in2;
	                    }
	                    out_m[i][j] = w;
	                }
	            }
...
//	        }
	    ...
	    private function PCA_PCA(o_output:FLARMat, o_ev:FLARVec):void {
	        ...
	        if (o_ev.getClm() != min) {//if (ev->clm != min) {
	       	...
	       	var u:FLARMat;//        u =new FLARMat(min, min);
	    ...
	    public function matrixPCA(o_evec:FLARMat, o_ev:FLARVec, mean:FLARVec):void {
	    	var srow:Number, sum:Number;
	    	var l_row:int, l_clm:int;
	    	var check:int;
